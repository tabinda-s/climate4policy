...
* break dataset into a few

keep if  idnum == 1 | idnum == 2 | idnum == 3 | idnum == 4 | idnum == 5 | idnum == 6 | idnum == 7 | idnum == 8 | idnum == 9 | idnum == 10 | idnum == 11 | idnum == 12 | idnum == 13 | idnum == 14 | idnum == 15 | idnum == 16 | idnum == 17 | idnum == 18 | idnum == 19 | idnum == 20 | idnum == 21 | idnum == 22 | idnum == 23 | idnum == 24 | idnum == 25 
replace county = "Mono" if idnum == 26
replace county = "Monterey" if idnum == 27
replace county = "Napa" if idnum == 28
...
bys idnum: egen mean_maxtemp_coeff = mean(coeff)
drop coeff*

* Other indicators
gen mean_tempcoeff = 0
...
foreach mnth in `months' {
foreach cnty in `counties' {
reg mean_temp year if month_str == "`mnth'" & county == `cnty'
gen mean_tempcoeff`cnty'`mnth' = _b[year]
replace mean_tempcoeff = coeff`cnty'`mnth' if county == `cnty' & month_str == "`mnth'"
}
}


*Now I need to average this change (over all the months) by county
bys idnum: egen mean_temp_change = mean(mean_tempcoeff)
...
foreach mnth in `months' {
foreach cnty in `counties' {
reg precip year if month_str == "`mnth'" & county == `cnty'
gen coeff`cnty'`mnth' = _b[year]
replace coeff = coeff`cnty'`mnth' if county == `cnty' & month_str == "`mnth'"
}
}
...
keep if county_tag
keep mean_coeff id idnum latitude longitude
...
export delimited using "maxtempbycounty.csv", replace
save temp/all_prism.dta, replace
